...
func (m *defaultFriendRequestsModel) FindByUidAndReqUid(ctx context.Context, userId, reqUid string) (*FriendRequests, error) {
	var resp FriendRequests
	query := fmt.Sprintf("select %s from %s where `user_id` = ? and `req_uid` = ? limit 1", friendRequestsRows, m.table)
	err := m.QueryRowNoCacheCtx(ctx, &resp, query, userId, reqUid)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
